...
use super::{
    abs_i32, add_entity_frame, add_hit_flash_timer, add_kill_count, add_money, add_pos_x, add_revive_count,
    add_revive_timer, advance_animation_frame, attack_dmg_dispatch, award_event_points, call_rng, can_push_back, cat_update,
    check_collision, get_anim_len, get_attacks_remaining, get_base_hp, get_battle_status, get_burrow_distance,
    get_burrow_start_x, get_cannon_base_damage, get_castle_id, get_castle_row, get_cat_combo_bonus, get_curse_timer,
    get_death_surge_anchor, get_death_surge_level, get_death_surge_mini, get_death_surge_span, get_enemy_money_drop,
    get_entity_button, get_entity_frame, get_entity_state, get_first_bounty_pct, get_freeze_timer, get_global_map_id,
    get_gudetama_soul, get_hit_flash_timer, get_hp, get_max_hp, get_no_revive, get_pos_x, get_revive_count, get_revive_hp,
    get_revive_time, get_revive_timer, get_sage_kb_resist_pct, get_score_time_limit, get_score_value, get_setting,
    get_shield_hp, get_shield_max, get_shield_regen, get_slow_timer, get_soul_anim_type, get_spawn_serial,
    get_special_rule, get_speed, get_treasure_value, get_unit_anim, get_weaken_timer, is_alien, is_angel, is_dark,
    is_floating, is_metal, is_red, is_score_stage, is_scored_stage, is_touchable_thunk, is_zombie, keep_in_bound,
    latch_battle_event, max_i32, min_i32, no_more_attacks, pending_strike_hit, play_sound, play_sound_in_battle,
    read_flag, roll_procs, scored_map_pays_money, set_crit_fx, set_drain_pct, set_entity_frame, set_entity_state,
    set_first_bounty_pct, set_frame_damage, set_hp, set_metal_killer_fx, set_prev_curse_timer, set_prev_freeze_timer,
    set_prev_slow_timer, set_prev_weaken_timer, set_proc_badge, set_revive_timer, set_savage_blow_fx,
    set_score_hit_mask, set_shield_fx, set_shield_fx_frame, set_shield_hp, set_shield_state, set_toxic_fx,
    slot_occupied, sound_manager, spawn_warp_tick, std_map_int_maanim_subscript_2, std_map_int_vector_erase,
    std_map_int_vector_subscript, std_vector_int_push_back, std_vector_int_push_back_2, AppContext, Base, Debris,
        ...
        'slot: {
            ...
            if slot == 0 {
                if get_base_hp(ctx, faction)? <= 0 {
                    ...
                    let ring = 0x32i32.wrapping_sub(counter.wrapping_sub((operation::div_5(counter as i64) as i32).wrapping_mul(5)));
                    ...
                    ctx.set_i32_at(AppContext::ENEMY_BASE_BLAST_X, x.wrapping_sub(half))?;
                    ...
                    let blast_x = ctx.i32_at(AppContext::ENEMY_BASE_BLAST_X)?;
            ...
            if ctx.i32_at(AppContext::CANNON_BLAST_ACTIVE)? == 1 && is_touchable_thunk(ctx, faction, slot as i32, -1)? {
            ...
            if get_entity_state(ctx, faction, slot as i32)? == 0 {
                ...
                if get_hp(ctx, faction, slot as i32)? != 0 {
                    ...
                    let restored = operation::div_100(get_shield_regen(ctx, faction, slot as i32)?.wrapping_mul(shield_max) as i64) as i32;

                    set_shield_hp(ctx, faction, slot as i32, restored)?;
                    set_shield_state(ctx, faction, slot as i32, 0)?;
                    set_shield_fx(ctx, faction, slot as i32, 3)?;
                    set_shield_fx_frame(ctx, faction, slot as i32, 0)?;
                ...
                if is_zombie(ctx, faction, slot as i32)? && get_revive_count(ctx, faction, slot as i32)? != 0 && !get_no_revive(ctx, faction, slot as i32)? {
                    ...
                    let revived = operation::div_100(get_revive_hp(ctx, faction, slot as i32)?.wrapping_mul(max_hp) as i64) as i32;
                ...
                if ctx.attackers_by_serial[credit].contains_key(&serial) {
                    let serial = get_spawn_serial(ctx, faction, slot as i32)?;
                    let attackers = std_map_int_vector_subscript(&mut ctx.attackers_by_serial[credit], &serial).to_vec();
                ...
                if is_scored_stage(ctx)? && get_battle_status(ctx)? != 4 {
                    ...
                    let weighted = limit.wrapping_add(limit).wrapping_sub(ctx.i32_at(AppContext::SCORE_ELAPSED)?).wrapping_mul(ctx.i32_at(entity.wrapping_add(Entity::SCORE_VALUE))?);
                    let time_bonus = operation::idiv(weighted, limit).ok_or(Fault::divide(SITE, limit as i64))?;
                    let occupant = ctx.i32_at(entity.wrapping_add(Entity::OCCUPANT))? as i64;
                    let drop = ctx.i32_at((occupant * ENEMY_STATS_STRIDE as i64 + (ENEMY_STATS + EnemyStats::CASH_DROP) as i64) as usize)?;
                    let score = (operation::div_100(drop as i64) as i32).wrapping_add(time_bonus).wrapping_add(ctx.i32_at(AppContext::SCORE_TOTAL)?);

                    ctx.set_i32_at(AppContext::SCORE_TOTAL, if score >= 0x3b9ac9ff { 0x3b9ac9ff } else { score })?;
                }
                ...
                let red = if is_red(ctx, faction, slot as i32)? { get_treasure_value(ctx, &ctx.treasure_store, 0xc)? } else { 0 };

                std_vector_int_push_back(&mut treasures, &red);

                let floating = if is_floating(ctx, faction, slot as i32)? { get_treasure_value(ctx, &ctx.treasure_store, 0xe)? } else { 0 };

                std_vector_int_push_back(&mut treasures, &floating);

                let dark = if is_dark(ctx, faction, slot as i32)? { get_treasure_value(ctx, &ctx.treasure_store, 0xd)? } else { 0 };

                std_vector_int_push_back(&mut treasures, &dark);

                let angel = if is_angel(ctx, faction, slot as i32)? { get_treasure_value(ctx, &ctx.treasure_store, 0xf)? } else { 0 };

                std_vector_int_push_back(&mut treasures, &angel);

                let metal = if is_metal(ctx, faction, slot as i32)? { get_treasure_value(ctx, &ctx.treasure_store, 0x13)? } else { 0 };

                std_vector_int_push_back(&mut treasures, &metal);

                let zombie = if is_zombie(ctx, faction, slot as i32)? { get_treasure_value(ctx, &ctx.treasure_store, 0x14)? } else { 0 };

                std_vector_int_push_back(&mut treasures, &zombie);

                let alien = if is_alien(ctx, faction, slot as i32)? { get_treasure_value(ctx, &ctx.treasure_store, 0x15)? } else { 0 };
                ...
                let boosted = (operation::div_1000(strongest.wrapping_mul(phase) as i64) as i32).wrapping_add(phase);
                let comboed = operation::div_100(combo.wrapping_add(0x64).wrapping_mul(boosted) as i64) as i32;
                let push = operation::div_neg_100(0x64i32.wrapping_sub(sage_resist).wrapping_mul(comboed) as i64) as i32;
                let x = ctx.i32_at(entity.wrapping_add(Entity::POS_X))?;
