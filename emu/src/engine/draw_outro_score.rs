...
use super::{
    button_bank_find, draw_context, draw_cut, draw_number_plain, fill_rect, get_drawable_width, get_stage_score, glow_set, imgcut_get_sprite_cut, is_score_stage,
    new_button_draw, set_tint, AppContext, OUTRO_SLIDE_TABLE,
};

const SITE: &str = "draw_outro_score";

fn slide(frame: i32) -> Result<i32, Fault> {
    OUTRO_SLIDE_TABLE
        .get(frame as i64 as usize)
        .copied()
        .ok_or(Fault::IndexOutOfRange { site: SITE, index: frame as i64, limit: OUTRO_SLIDE_TABLE.len() as i64 })
}

fn outro_buttons(ctx: &mut AppContext) -> Result<(), Fault> {
...
pub fn draw_outro_score(ctx: &mut AppContext) -> Result<(), Fault> {
    ...
    let left = 0i32.wrapping_sub(slide(step)?);
    ...
    let x = operation::cvttss2si(left - cut as f32 + -20.0);

    draw_cut(draw_context(&mut ctx.draw)?, banner, x, 0x144, 0xb);

    let x = operation::cvttss2si(20.0 + right);

    draw_cut(draw_context(&mut ctx.draw)?, banner, x, 0x143, 4);

    if ctx.i32_at(AppContext::OUTRO_PHASE)? == 2 {
        return outro_buttons(ctx);
    }

    if ctx.i32_at(AppContext::NEW_BEST_SCORE)? != 1 {
        return outro_buttons(ctx);
    }
    ...
    let blink = (operation::div_2(phase as i8 as i32) as i8).wrapping_add(6) as u8;
