...
pub fn get_item_count(ctx: &AppContext, item: i32) -> Result<i32, Fault> {
    ...
    loop {
        ...
        let redirect = ctx.i32_at(AppContext::ITEM_DEFINITIONS.wrapping_add(definition).wrapping_add(ItemDefinition::REDIRECT))?;
    ...
    let kind_at = AppContext::ITEM_DEFINITIONS.wrapping_add(definition).wrapping_add(ItemDefinition::KIND);
    let index_at = AppContext::ITEM_DEFINITIONS.wrapping_add(definition).wrapping_add(ItemDefinition::INDEX);
    ...
    if ctx.i32_at(kind_at)? == 5 {
        ...
        let cell = ctx.block_at::<8>(AppContext::ITEM_COUNTS_KIND_5.wrapping_add((index * 8) as usize))?;
    ...
    if ctx.i32_at(kind_at)? == 6 {
        let index = ctx.i32_at(index_at)? as i64;
        let cell = ctx.block_at::<8>(AppContext::ITEM_COUNTS_KIND_6.wrapping_add((index * 8) as usize))?;
    ...
    if ctx.i32_at(kind_at)? == 7 {
        let index = ctx.i32_at(index_at)? as i64;
        let cell = ctx.block_at::<8>(AppContext::ITEM_COUNTS_KIND_7.wrapping_add((index * 8) as usize))?;
    ...
    if ctx.i32_at(kind_at)? == 3 {
        ...
        return Ok(obf_value_read_2(&ctx.block_at::<8>(AppContext::ITEM_COUNTS_KIND_3.wrapping_add((index * 8) as usize))?) as i32);
    }

    if ctx.i32_at(kind_at)? == 8 {
        let index = ctx.i32_at(index_at)? as i64;

        return Ok(obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_COUNTS_KIND_8.wrapping_add((index * 8) as usize))?) as i32);
    }
    ...
    if item > 0x7a {
        match item {
            ...
            0x91 => return Ok(obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_91_COUNT)?) as i32),
            0x9d => return Ok(obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_9D_COUNT)?) as i32),
            _ => {}
        }
    } else {
        match item {
            0x1d => return Ok(obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_1D_COUNT)?) as i32),
            0x5c => return Ok(obf_value_read_2(&ctx.block_at::<8>(AppContext::ITEM_5C_COUNT)?) as i32),
    ...
    if ctx.i32_at(kind_at)? == 9 {
        ...
        return Ok(ctx.i8_at(AppContext::ITEM_COUNTS_KIND_9.wrapping_add((index * AppContext::ITEM_COUNTS_KIND_9_STRIDE as i64) as usize))? as i32);
    }
    ...
    if ctx.i32_at(kind_at)? == 0xa {
        ...
        if index >= 2 {
            return Err(Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: 2 });
        }

        return Ok(obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_COUNTS_KIND_A.wrapping_add((index * 8) as usize))?) as i32);
    }
    ...
    if ctx.i32_at(kind_at)? == 0xb {
        ...
        if index >= 4 {
            return Err(Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: 4 });
        }
    ...
    if ctx.i32_at(kind_at)? == 0xc {
        ...
        if index >= 0x2a {
            return Err(Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: 0x2a });
        }

        return Ok(obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_COUNTS_KIND_C.wrapping_add((index * 8) as usize))?) as i32);
    }
