...
pub fn skill_wave_invalid_draw(
    ctx: &mut AppContext,
    x: i32,
    y: i32,
    frame: i32,
    ...
    let (model, anim) = if faction != 0 {
        (
            &mut ctx.skill_wave_invalid_e_model,
            &ctx.skill_wave_invalid_e_anim,
        )
    } else {
        (
            &mut ctx.skill_wave_invalid_model,
            &ctx.skill_wave_invalid_anim,
        )
    };
