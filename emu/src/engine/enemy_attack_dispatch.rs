...
use super::{
    add_score_hit_mask, attack_dmg_dispatch, attack_proc_dispatch, battle_not_finishing, build_trait_mask, call_rng, count_target_traits, does_target,
    ...
    get_weaken_timer, has_behemoth_slayer, has_colossus_slayer, has_conjure_deck_slot, has_eva_killer, has_fixed_lineup, has_insanely_tough, has_resist,
    has_sage_slayer, has_strong_against, has_witch_slayer, is_alien, is_aku, is_angel, is_behemoth, is_colossus, is_dark, is_eva_angel, is_floating,
    is_metal, is_red, is_relic, is_sage, is_touchable_thunk, is_traitless, is_witch, is_zombie, max_i32, set_barrier_state, set_behemoth_dodge_timer,
    set_crit_fx, set_dodge_fx_frame, set_dodge_timer, set_orb_dodge_timer, set_savage_blow_fx, set_shield_state, set_toxic_fx, start_immune_fx,
    AppContext, Entity,
};
...
pub fn enemy_attack_dispatch(
    ...
    if savage != 0 {
        ...
        damage = damage.wrapping_mul(boost.wrapping_add(100) as i64);
        set_savage_blow_fx(ctx, 0, target, 1)?;
        damage = operation::div_100(damage);
    }
    ...
    if target == 0 {
        ...
        if is_behemoth(ctx, 1, attacker)? {
            ...
            if roll < get_behemoth_dodge_chance(ctx, 0, target)? {
                ...
                set_dodge_fx_frame(ctx, 0, target, 1)?;
        ...
        if does_target(ctx, 0, target, attacker)? || has_conjure_deck_slot(ctx, 0, target)? {
            ...
            if roll < get_dodge_chance(ctx, 0, target)? {
                ...
                let duration = operation::div_1500(get_dodge_duration(ctx, 0, target)?.wrapping_mul(treasure.wrapping_add(0x5dc)));
            ...
            if has_strong_against(ctx, 0, target)? {
                ...
                damage = operation::div_3000(damage.wrapping_mul(0x5dci32.wrapping_sub(treasure) as i64));
                damage = operation::div_100((100i32.wrapping_sub(combo) as i64).wrapping_mul(damage));
            ...
            if has_resist(ctx, 0, target)? {
                ...
                damage = operation::div_6000(damage.wrapping_mul(0x5dci32.wrapping_sub(treasure) as i64));
                damage = operation::div_100((100i32.wrapping_sub(combo) as i64).wrapping_mul(damage));
                damage = (100i32.wrapping_sub(orb) as i64).wrapping_mul(damage);
        ...
        if has_witch_slayer(ctx, 0, target)? && is_witch(ctx, 1, attacker)? {
            ...
            damage = operation::div_10(damage).wrapping_mul(operation::div_neg_100(combo).wrapping_add(5) as i64);
        ...
        if has_eva_killer(ctx, 0, target)? && is_eva_angel(ctx, 1, attacker)? {
            ...
            damage = operation::div_5(damage).wrapping_mul(operation::div_neg_100(combo).wrapping_add(5) as i64);
        ...
        if has_behemoth_slayer(ctx, 0, target)? && is_behemoth(ctx, 1, attacker)? {
            let permille = get_setting(&ctx.settings, b"battle_super_beast_hunter_df", 0x2bc)? as i64;
        ...
        if has_sage_slayer(ctx, 0, target)? && is_sage(ctx, 1, attacker)? {
            let permille = get_setting(&ctx.settings, b"battle_super_sage_hunter_damage2", 0x2bc)? as i64;
        ...
        if kaijin_combo > 0 && get_trait_kaijin(ctx, 1, attacker)? {
            damage = operation::div_wide(damage.wrapping_mul(1000), kaijin_combo as u32 as i64).ok_or(Fault::divide("enemy_attack_dispatch", kaijin_combo as i64))?;
        }
    ...
    if target != 0 && targets as u8 & p_toxic != 0 {
        if get_toxic_immune(ctx, 0, target)? {
            start_immune_fx(ctx, 0, target)?;
        } else {
            let max_hp = get_max_hp(ctx, 0, target)?;
            let mut toxic = max_i32(operation::div_100(get_toxic_damage(ctx, 1, attacker)?.wrapping_mul(max_hp)), 1);
            let resist = get_toxic_resist_pct(ctx, 0, target)?;

            toxic = max_i32(operation::div_100((100i32.wrapping_sub(resist) as i64).wrapping_mul(toxic as i64)) as i32, 1);

            let reduction = get_cannon_effect(get_resist_part_rec(ctx)?, 0xcd, style_level)?;
            let mut toxic = toxic as i64;

            if reduction != 0 {
                toxic = operation::div_10000((10000i32.wrapping_sub(reduction) as i64).wrapping_mul(toxic));
            }

            set_toxic_fx(ctx, 0, target, 1)?;
    ...
    if immune {
        ctx.set_i32_at(AppContext::entity_field(0, target, Entity::WAVE_IMMUNE_FX_FRAME), 0)?;
        ctx.set_i32_at(AppContext::entity_field(0, target, Entity::WAVE_IMMUNE_FX_ACTIVE), 1)?;
        dealt = 0;
    }
