...
use super::{
    AppContext, get_stage_count, get_stage_record, get_user_rank, is_map_cleared,
...
pub fn unlock_condition_met(ctx: &mut AppContext, id: i32) -> Result<bool, Fault> {
    let slot = match id {
        ...
        26 => {
            return Ok(is_map_cleared(ctx, 1, 0xd, 0, 0)?
                && is_map_cleared(ctx, 1, 0x1a, 0, 0)?
                && is_map_cleared(ctx, 1, 0x1d, 0, 0)?);
        }
        27 => {
            return Ok(is_map_cleared(ctx, 1, 0x1e, 0, 0)?
                && is_map_cleared(ctx, 1, 0x1f, 0, 0)?
                && is_map_cleared(ctx, 1, 0x20, 0, 0)?);
        }
        28 => {
            return Ok(is_map_cleared(ctx, 1, 9, 0, 0)?
                && is_map_cleared(ctx, 1, 0x24, 0, 0)?
                && is_map_cleared(ctx, 1, 0x26, 0, 0)?);
        }
        29 => {
            return Ok(is_map_cleared(ctx, 1, 0xa, 0, 0)?
                && is_map_cleared(ctx, 1, 0xb, 0, 0)?
                && is_map_cleared(ctx, 1, 0xc, 0, 0)?);
        }
        _ => {
            if id == 0xf423f {
                return Ok(false);
            ...
            if map_id as u32 <= 0x1869f {
                return is_map_cleared(
                    ctx,
                    map_type_of_map_id(map_id),
                    map_index_of_map_id(map_id),
                    0,
                    0,
                );
            }
