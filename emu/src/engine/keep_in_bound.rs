...
pub fn keep_in_bound(ctx: &mut AppContext, faction: i32, slot: i32) -> Result<(), Fault> {
    ...
    if faction == 1 {
        if ctx.i32_at(AppContext::entity_field(1, slot, Entity::BACK_BOUND))?
        ...
        if ctx.i32_at(AppContext::entity_field(0, slot, Entity::BACK_BOUND))?
    ...
    ctx.set_i32_at(
        AppContext::entity_field(faction, slot, Entity::POS_X),
        bound,
    )
}
