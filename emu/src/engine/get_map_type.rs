...
pub fn get_map_type(ctx: &mut AppContext, base_only: u8) -> Result<i32, Fault> {
    ...
    if ctx.i32_at(0x327efc)? >= 4 && ctx.i32_at(0x327efc)? <= 6 {
        let outbreak = is_outbreak_stage(ctx)?;
    ...
    if ctx.i32_at(0x327efc)? >= 7 && ctx.i32_at(0x327efc)? <= 9 {
    ...
    Ok(if ctx.i32_at(0x327efc)? != 0x63 { -1 } else { -8 })
}
