        ...
        'spent: {
            if conjure {
                if !conjurer_on_field(ctx, faction, slot, 1)? {
                    if get_deck_cooldown(ctx, wallet, slot)? == 0
                        && ctx.u8_at(
                            wallet
                                .wrapping_add(slot as i64 as usize)
                                .wrapping_add(AppContext::WALLET_SPIRIT_USED),
                        )? != 0
                    {
                        ctx.set_i32_at(AppContext::DEPLOY_NOTICE_TIMER, 0x1e)?;
                ...
                if level
                    >= get_unit_max_level(ctx, get_button_unit_id(ctx, faction, spirit_button)?)?
                {
                    level =
                        get_unit_max_level(ctx, get_button_unit_id(ctx, faction, spirit_button)?)?
                            .wrapping_sub(1);
                ...
                while scan != 51 {
                    if slot_occupied(ctx, 0, scan)? == 0
                        || get_slot_unit_id(ctx, faction, scan)?
                            != get_button_unit_id(ctx, faction, slot)?
                    {
                        scan += 1;
                    ...
                    let mut x = get_pos_x(ctx, faction, scan)?.wrapping_sub(
                        get_setting(&ctx.settings, b"summon_position", 0)?.wrapping_shl(2),
                    );
            ...
            if faction == 0 && spawned >= 0 {
                    ...
                    if (mask as u32 >> (rarity as u32 & 0x1f)) & 1 != 0 {
                        ...
                        loop {
                            ...
                            ctx.deploy_queue.push(
                                ((gap.wrapping_mul(queued) as u32 as u64) << 0x20)
                                    .wrapping_add(slot as u32 as u64),
                            );
                        }
                    }
                }
                ...
                if is_score_stage(ctx.event_items.as_ref()) {
                    ...
                    let listed_cost = ctx.i32_at(
                        ((unit_id.wrapping_add(2) as i64) * CAT_STATS_UNIT_STRIDE as i64
                            + form * CAT_STATS_FORM_STRIDE as i64
                            + (CAT_STATS + CatStats::EOC1_COST) as i64)
                            as usize,
                    )?;
                    let boosted = ctx
