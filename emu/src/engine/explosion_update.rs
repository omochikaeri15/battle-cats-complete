...
pub fn explosion_update(ctx: &mut AppContext) -> Result<(), Fault> {
    ...
    while (event_index as i64 as usize) < ctx.explosion_events.len() {
        ...
        let missing = Fault::IndexOutOfRange { site: SITE, index: event_index as i64, limit: ctx.explosion_events.len() as i64 };
        ...
        let rings = min_i32(operation::idiv(frame, interval).ok_or(Fault::divide(SITE, interval as i64))?.wrapping_add(1), 3);
        ...
        if rings > 0 {
            ...
            while ring != rings as u32 as usize {
                ...
                let default_width = *EXPLOSION_WIDTH_DEFAULTS.get(ring).ok_or(Fault::IndexOutOfRange { site: SITE, index: ring as i64, limit: 3 })?;
                let width = get_setting(&ctx.settings, &[b"battle_explosion_width".as_slice(), number.as_bytes()].concat(), default_width)?;
                let mut spread = 0i32;
                ...
                match ring {
                    2 => {
                        ...
                        spread = width1.wrapping_add(width2.wrapping_mul(2)).wrapping_add(width3);
                    }
                    1 => {
                        let width1 = get_setting(&ctx.settings, b"battle_explosion_width1", 0x320)?;
                ...
                let active = if elapsed >= get_setting(&ctx.settings, b"battle_explosion_frame1", 0xf)? {
                    let start = get_setting(&ctx.settings, b"battle_explosion_frame1", 0xf)?;

                    elapsed < get_setting(&ctx.settings, b"battle_explosion_frame2", 0x14)?.wrapping_add(start)
                } else {
                    false
                };
                ...
                while slot != 51 {
                    ...
                    let x = if faction == 1 {
                        get_pos_x(ctx, 0, slot)?.wrapping_add(ctx.i32_at(AppContext::entity_field(0, slot, Entity::HITBOX_POS))?)
                    } else {
                        get_pos_x(ctx, other, slot)?.wrapping_sub(ctx.i32_at(AppContext::entity_field(other, slot, Entity::HITBOX_POS))?)
                    };
                    let mut pass = 0i32;
                    let mut first = true;
                    ...
                    loop {
                        'pass: {
                            ...
                            if event.hits.contains_key(&slot) {
                                let flags = event.hits.get(&slot).ok_or(Fault::KeyNotFound { site: SITE, key: slot as i64 })?;
                            ...
                            let center = pass.wrapping_mul(spread).wrapping_add(left).wrapping_add(event.x);
                            ...
                            let default_damage = *EXPLOSION_DAMAGE_DEFAULTS.get(ring).ok_or(Fault::IndexOutOfRange { site: SITE, index: ring as i64, limit: 3 })?;
                            let dmg_scale = get_setting(&ctx.settings, &[b"battle_explosion_damage".as_slice(), number.as_bytes()].concat(), default_damage)?;

                            explosion_attack(ctx, event_index, slot, attack, dmg_scale)?;

                            let flags = ctx.explosion_events.get_mut(at).ok_or(missing.clone())?.hits.entry(slot).or_insert([0; 5]);

                            *flags.get_mut(hit).ok_or(Fault::IndexOutOfRange { site: SITE, index: hit as i64, limit: 5 })? = 1;
                        }
        ...
        if frame >= start.wrapping_add(interval.wrapping_mul(2)).wrapping_add(burn).wrapping_add(fade) {
