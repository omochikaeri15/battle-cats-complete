...
use super::{get_drawable_width, get_left_inset_logical, get_right_inset_logical, get_top_inset_offset, AppContext};
...
pub fn reset_hud_corner_rects(ctx: &mut AppContext) -> Result<(), Fault> {
    ...
    ctx.set_i32_at(AppContext::CANNON_RECT, width.wrapping_sub(right).wrapping_add(-0x92))?;
