...
use super::{ATTACK_LD_ANCHOR_FIELDS, ATTACK_LD_FLAG_FIELDS, ATTACK_LD_SPAN_FIELDS, AppContext, Entity};

pub fn get_ld_span(ctx: &AppContext, faction: i32, slot: i32, attack: i32) -> Result<i32, Fault> {
    let marker = 'resolved: {
        if attack > 0 {
            let flag_field = *ATTACK_LD_FLAG_FIELDS
                .get(attack as usize)
                .ok_or(Fault::IndexOutOfRange { site: "get_ld_span", index: attack as i64, limit: 3 })?;
    ...
    let field = *ATTACK_LD_SPAN_FIELDS
        .get(source as usize)
        .ok_or(Fault::IndexOutOfRange { site: "get_ld_span", index: source as i64, limit: 3 })?;
