...
pub fn trial_win_update(ctx: &mut AppContext) -> Result<bool, Fault> {
    ...
    let frame = ctx.i32_at(AppContext::OUTRO_FRAME)?.wrapping_add(1);

    ctx.set_i32_at(AppContext::OUTRO_FRAME, frame)?;
    ctx.set_i32_at(
    ...
    if phase == 3 {
        ...
        ctx.set_i32_at(AppContext::OUTRO_OK_SLIDE, slide)?;

        let limit = 0x280i32.wrapping_sub(ctx.deck_bar_base_y);

        if slide >= limit {
            ctx.set_i32_at(AppContext::OUTRO_OK_SLIDE, limit)?;
