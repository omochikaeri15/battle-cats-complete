...
use super::{
    get_base_upgrade, get_button_unit_form, get_button_unit_id, get_cat_combo_bonus, get_global_map_id, get_special_rule_params, get_treasure_value, get_unit_rarity,
...
pub fn get_unit_recharge(ctx: &mut AppContext, faction: i32, button: i32) -> Result<i32, Fault> {
    ...
    if let Some(params) = get_special_rule_params(ctx, &ctx.special_rules, map_id, 1)? {
        return params.first().copied().ok_or(Fault::IndexOutOfRange { site: SITE, index: 0, limit: 0 });
    }
    ...
    let reduced = operation::div_neg_100(combo.wrapping_mul(reduction)).wrapping_add(cooldown.wrapping_sub(reduction));
    ...
    if let Some(params) = get_special_rule_params(ctx, &ctx.special_rules, map_id, 6)? {
        ...
        let percent = *params.get(rarity as i64 as usize).ok_or(Fault::IndexOutOfRange { site: SITE, index: rarity as i64, limit: params.len() as i64 })?;
