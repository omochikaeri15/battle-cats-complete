...
pub fn stat_attack_ld_span(ctx: &AppContext, faction: i32, unit_id: i32, form: i32, attack: i32) -> Result<i32, Fault> {
    let cell = if read_flag(ctx, (faction as usize).wrapping_mul(0x1f0).wrapping_add(0x2648))? & 1 != 0 {
        let column = *STAT_ATTACK_COLUMNS.get((30 + attack as i64) as usize).ok_or(Fault::IndexOutOfRange {
            site: "stat_attack_ld_span",
            index: attack as i64,
            limit: 3,
        })?;
        ...
        let column = *STAT_ATTACK_COLUMNS.get((33 + attack as i64) as usize).ok_or(Fault::IndexOutOfRange {
            site: "stat_attack_ld_span",
            index: attack as i64,
            limit: 3,
        })?;
