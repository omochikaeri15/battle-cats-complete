...
pub fn clear_lineup_record(ctx: &mut AppContext, map: i32, stage: i32, star: i32) -> Result<(), Fault> {
    let (map, stage, star) = if map == -1 { (get_global_map_id(ctx, 0)?, get_stage_index(ctx)?, get_star_level(ctx)?) } else { (map, stage, star) };
    let key = star.wrapping_add(stage.wrapping_mul(10)).wrapping_add(map.wrapping_mul(0x3e8));
