...
pub fn game_win_update(ctx: &mut AppContext) -> Result<bool, Fault> {
        ...
        match kind {
            ...
            10 => {
                ...
                message = substitute_tokens(
                    ctx,
                    &text,
                    &[(b"stageName1", &first), (b"stageName2", &second)],
                )?;
            ...
            12 => {
                ...
                if item & !0x10 != 6 {
                    ...
                    possession = substitute_tokens(
                        ctx,
                        &text,
                        &[(b"itemNum1", &first), (b"itemNum2", &second)],
                    )?;
                ...
                let key: &[u8] = if item == 6 || item == 0xf7 || item == 0xcf {
                    b"drop_popup_xp"
                } else {
                    b"drop_popup_item"
                };
    ...
    if phase == 6 {
        ...
        'offer: {
            ...
            if call_rng(ctx, 100) < ctx.i32_at(AppContext::STAGE_EX_CHANCE)? {
                ...
                let span = ctx
                    .i32_at(AppContext::STAGE_EX_STAGE_MAX)?
                    .wrapping_sub(low)
                    .wrapping_add(1);
                ...
                ctx.set_i32_at(
                    AppContext::EX_STAGE,
                    key.wrapping_sub(map.wrapping_mul(100)),
                )?;
            } else {
                break 'offer;
            }
            ...
            let dialog =
                dialog_show_kind2(ctx, &message, 0, 0, 4, Some(game_win_update_lambda_16))?;
    ...
    if old.wrapping_add(1) == 0x27 {
        ...
        let message =
            substitute_tokens(ctx, &text, &[(b"rankingNum", &rank), (b"stageName", &name)])?;
        ...
        dialog_show_alt(
            ...
            0x6e,
            0x140,
            Some(game_win_update_lambda_19),
        )?;
