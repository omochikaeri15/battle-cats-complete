...
use super::{
    analytics_params, clear_cannon_shot, clear_crit_fx_slot, clear_debris, clear_effect_slot, clear_wave_sprite, app_on_draw, bgm_player_switch, button_bank_remove, calculate_treasure_percentages, clear_items_selected, fever_clear_state,
    get_battle_status, get_entity_base_idx, get_entity_state, get_global_map_id, get_item_selected, get_max_hp, get_max_money, get_powerup_available,
    get_scene_id, get_stage_index, get_star_level, load_map_stage_csv, lose_exit_map_check, map_type_base_id, notification_schedule, record_stage_played,
    request_save_data, reset_hud_corner_rects, scene_transition_tick, set_battle_status, set_cannon_countdown, set_deck_cooldown, set_entity_state, set_hp,
    set_item_selected, set_money, set_powerup_available, set_scene, set_worker_level, sound_manager, validate_map_type, vibration_clear, AppContext, Entity,
    FormatArg, ENTITY_BASE, ENTITY_STRIDE, FACTION_STRIDE,
};
...
pub fn fade_update(ctx: &mut AppContext, style: i32) -> Result<bool, Fault> {
    ...
    if style == 0 {
        ...
        if frame == 0 {
            ...
            if frame == 0xb && started != 0 && get_scene_id(ctx)? == 0x62 && ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.fade_menu_prompt() {
                app_on_draw(ctx)?;
        ...
        if ctx.i32_at(AppContext::FADE_FRAME)? == 0xd {
            ...
            if scene == 0x64 || scene == 0x62 {
                ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.fade_menu_dispatch(0, scene);
            }
        }
    ...
    if frame == 0 {
    ...
    } else if frame == 0xa && ctx.u8_at(AppContext::FADE_STARTED)? != 0 && get_scene_id(ctx)? == 0x12c {
        let status = get_battle_status(ctx)?;
        let collab = status == 1 && ctx.i32_at(AppContext::CHAPTER_MODE)? == 3 && ctx.i32_at(AppContext::STAR_LEVEL)? == 0 && {
            let map = map_type_base_id(validate_map_type(ctx.i32_at(AppContext::SAVED_MAP_TYPE)?), ctx.i32_at(AppContext::MAP_INDEX)?);
            let stage = ctx.i32_at(AppContext::STAGE_ROW)?;

            ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.collab_reward_ready(map, stage)
        };
        ...
        if collab {
            if ctx.dialogs.active.is_empty() {
                ...
                ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.collab_reward_dialog(map, stage);
            }

            step = Step::Stop;
        } else {
            let blocked = get_battle_status(ctx)? != 0
                && (get_battle_status(ctx)? != 1 || ctx.i32_at(AppContext::RESULT_MAP_LOCKED)? != 0)
                && (get_battle_status(ctx)? != 2 || ctx.i32_at(AppContext::REVIVE_REQUESTED)? != 0);

            if !blocked && ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.unlock_popup_pending() {
                step = Step::Stop;
            }
        }
    }
    ...
    loop {
        match step {
            Step::Advance => {
                ...
                if frame == 0xd && ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.loader_busy() {
                    step = Step::Stop;
            ...
            Step::TreasureCalc => {
                calculate_treasure_percentages(ctx)?;
                step = if ctx.i32_at(AppContext::FADE_FRAME)? != 0xc { Step::Tail } else { Step::Dispatch };
            }
            Step::Tail => {
            ...
            Step::ResumeRevive => {
                ...
                for offset in (0..0xc80usize).step_by(0x10) {
                    clear_crit_fx_slot(ctx, AppContext::CRIT_FX + offset)?;
                }
                ...
                for slot in 1..0x31 {
                    if slot != base {
                        ctx.set_i32_at(ENTITY_BASE + FACTION_STRIDE + slot as usize * ENTITY_STRIDE + Entity::POS_X, 0xaf0)?;
                    }
                }

                if base != 0x31 {
                    ctx.set_i32_at(ENTITY_BASE + FACTION_STRIDE + 0x31 * ENTITY_STRIDE + Entity::POS_X, 0xaf0)?;
                }

                if base != 0x32 {
                    ctx.set_i32_at(ENTITY_BASE + FACTION_STRIDE + 0x32 * ENTITY_STRIDE + Entity::POS_X, 0xaf0)?;
                }
            ...
            Step::Dispatch => {
                ...
                if scene == 0x3e7 {
                    ...
                    step = if get_battle_status(ctx)? == 2 && ctx.i32_at(AppContext::REVIVE_REQUESTED)? == 1 { Step::ResumeRevive } else { Step::SaveStop };
                ...
                if scene != 0x12c {
                    if scene == 0x63 || scene == 0x64 {
                        if ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.fade_menu_dispatch(1, scene) {
                            step = Step::Tail;
                ...
                if get_battle_status(ctx)? | ctx.i32_at(AppContext::TUTORIAL_CLEARED)? == 0 {
                    ...
                    if chapter == 0x63 {
                        ctx.set_i32_at(AppContext::CHAPTER_MODE, ctx.i32_at(AppContext::RESULT_CHAPTER_MODE)?)?;
                        ctx.set_i32_at(AppContext::ENTRY_STAGE, ctx.i32_at(AppContext::RESULT_ENTRY_STAGE)?)?;
                    }
                    ...
                    ctx.set_i32_at(AppContext::SCENE_0X64_PAGE_NEXT, -1)?;
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.battle_exit_cleanup((chapter == 0x63) as u8);
                ...
                if status == 1 {
                    ...
                    if ctx.i32_at(AppContext::RESULT_MAP_LOCKED)? != 0 {
                        clear_items_selected(ctx)?;
                        ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_ui_reset();
                        set_scene(ctx, 0x63)?;
                        scene_transition_tick(ctx)?;
                    ...
                    if ctx.i32_at(AppContext::REVIVE_REQUESTED)? != 0 && get_battle_status(ctx)? != 0 {
                        ...
                        for button in 0..10usize {
                            ...
                            ctx.set_i32_at(AppContext::DECK_COOLDOWN_FX + button * 4, 0)?;
                        }
                    ...
                    if get_battle_status(ctx)? == 0 && ctx.i32_at(AppContext::BATTLE_INTRO_FRAME)? >= 0x14a {
                    ...
                    if get_battle_status(ctx)? == 0 {
                        ...
                        ctx.set_i32_at(AppContext::BATTLE_CONTINUED, 0)?;
                        ctx.set_i32_at(AppContext::BATTLE_RESUMED, 0)?;
                    } else if get_battle_status(ctx)? == 2 && ctx.u8_at(AppContext::LEADERSHIP_REFUND)? != 0 {
                        ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.leadership_refund();
                    ...
                    ctx.set_i32_at(AppContext::SCENE_0X64_PAGE, direct.wrapping_mul(4).wrapping_add(5))?;
                    ctx.set_i32_at(AppContext::SCENE_0X64_PAGE_NEXT, -1)?;
                    button_bank_remove(&mut ctx.buttons, 0xc8);
                    button_bank_remove(&mut ctx.buttons, 0xc9);
                    ...
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_return_reset();
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_return_flags();
                    ...
                    ctx.set_block_at::<8>(AppContext::MENU_TEXTURE_PAGE + 0x30, 0x1ca00000000u64.to_le_bytes())?;
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_screen_init(0);
                    ...
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_menu_build(1);
                    ctx.set_i32_at(AppContext::MENU_BUILD_MODE, 0)?;
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_background_pick();
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.scene_background_setup();
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_ui_reset();
                    sound_manager(ctx)?.stop_audio(-1);
                    ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.map_bgm_start(0);
                    step = Step::Stop;
                    ...
                    if ctx.i32_at(AppContext::RESULT_MAP_LOCKED)? != 0 || ctx.u8_at(AppContext::CAT_FOOD_SHOP_OPEN)? != 0 {
                        return Ok(false);
                    }
                } else {
                    clear_items_selected(ctx)?;
                ...
                if ctx.u8_at(AppContext::EX_ACCEPTED)? != 0 {
                    ...
                    if load_map_stage_csv(ctx, ex_map, 0, 1, 0, 1, 1)? {
                        ...
                        if status == 1 {
                            ctx.scene_host().ok_or(Fault::HostMissing { site: SITE })?.leadership_return_begin();
                        }
