...
const SITE: &str = "get_right_inset_logical";
...
pub fn get_right_inset_logical(ctx: &AppContext) -> Result<i32, Fault> {
    ...
    let drawable = if metrics.inset_left != 0 || metrics.inset_right != 0 {
        let dividend = metrics.screen_w.wrapping_sub(metrics.inset_left.wrapping_add(metrics.inset_right)).wrapping_mul(metrics.design_h);
        let divisor = metrics.screen_h.wrapping_sub(metrics.inset_top.wrapping_add(metrics.inset_bottom));
    ...
    let left = operation::cvttss2si(((width as f32) * (metrics.inset_left as f32) / (metrics.screen_w as f32)).ceil());
