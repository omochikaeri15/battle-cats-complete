...
use super::{call_rng, get_background_id, get_drawable_width, AppContext};
...
pub fn background_particles_init(ctx: &mut AppContext) -> Result<(), Fault> {
        ...
        || get_background_id(ctx)? == 0x432
    {
        ...
        while particle != 100 {
            ...
            let span = operation::div_10(ctx.i32_at(AppContext::STAGE_LENGTH)?).wrapping_add(get_drawable_width(ctx)?.wrapping_mul(4)).wrapping_add(-0xf00);

            ctx.set_i32_at(record, operation::irem(roll, span).ok_or(Fault::divide(SITE, span as i64))?)?;

            if get_background_id(ctx)? == 2 || get_background_id(ctx)? == 0x1a || get_background_id(ctx)? == 0x1b || get_background_id(ctx)? == 0x432 {
                let depth = 100i32.wrapping_sub(ctx.i32_at(AppContext::CAMERA_MIN_ZOOM)?).wrapping_mul(20);
    ...
    if get_background_id(ctx)? == 3 || get_background_id(ctx)? == 0x1b || get_background_id(ctx)? == 0xc5 {
        ...
        while drifter != 100 {
            ...
            let x = x.wrapping_add(get_drawable_width(ctx)?.wrapping_mul(4)).wrapping_mul(100).wrapping_add(-0x5dc00);
        ...
        || get_background_id(ctx)? == 0xa9
    {
        ...
        while particle != 100 {
            ...
            let y = if rising != 0 {
                100i32.wrapping_sub(ctx.i32_at(AppContext::CAMERA_MIN_ZOOM)?).wrapping_mul(20).wrapping_neg()
            } else {
                let reach = 0x280i32.wrapping_sub(ctx.i32_at(AppContext::BATTLE_ZOOM_Y)?);
                let spread = 100i32.wrapping_sub(ctx.i32_at(AppContext::CAMERA_MIN_ZOOM)?).wrapping_mul(reach);

                if spread > 0x1f3ff {
                    0
                } else {
                    call_rng(ctx, operation::div_neg_100(spread).wrapping_add(0x500))
                }
            };
            ...
            if get_background_id(ctx)? == 0x51 || get_background_id(ctx)? == 0x65 || get_background_id(ctx)? == 0x7b || get_background_id(ctx)? == 0x92 || get_background_id(ctx)? == 0xa9 {
    ...
    if get_background_id(ctx)? == 0x28 {
        ...
        while pair != 100 {
            ...
            for half in [0usize, 0x20] {
                ...
                ctx.set_i32_at(sprite + half + 4, roll.wrapping_sub(depth).wrapping_mul(100))?;
        ...
        while pair != 100 {
            ...
            loop {
                ...
                let (bound, base) = if first {
                    let reach = (-0x122i32).wrapping_sub(100i32.wrapping_sub(zoom).wrapping_mul(20));
                    let y = operation::div_100(call_rng(ctx, 100).wrapping_mul(reach)).wrapping_mul(100).wrapping_add(0x7148);

                    ctx.set_i32_at(sprite + 4, y)?;

                    let pick = call_rng(ctx, 10);

                    ctx.set_i32_at(sprite + 0xc, 0x14i32.wrapping_add(((pick as u32) < 1) as i32))?;

                    (0x32, 0x32)
                } else {
                    let lift = operation::idiv(0x4e20, zoom).ok_or(Fault::divide(SITE, zoom as i64))?;
                    let reach = lift.wrapping_add(100i32.wrapping_sub(zoom).wrapping_mul(20)).wrapping_add(0x1e0).wrapping_neg();
                    let y = operation::div_100(call_rng(ctx, 100).wrapping_mul(reach)).wrapping_add(lift).wrapping_add(0x1e0).wrapping_mul(100);
    ...
    if get_background_id(ctx)? == 0x37 {
        ...
        while particle != 100 {
            ...
            ctx.set_i32_at(AppContext::BG_PARTICLES.wrapping_add(particle * 0x14), spark)?;
            particle += 1;
        }
    }
