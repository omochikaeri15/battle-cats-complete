...
use super::{get_base_upgrade, get_castle_hp_growth, get_cat_combo_bonus, get_treasure_value, has_fixed_lineup, AppContext};
...
pub fn compute_base_health(ctx: &mut AppContext) -> Result<i32, Fault> {
    ...
    let base = if first <= 3 {
        ...
        if second <= 7 { third.wrapping_mul(2000).wrapping_add(-2000) } else { third.wrapping_mul(3000).wrapping_add(-9000) }
    };
    let level = if has_fixed_lineup(ctx, -1, -1, -1)? && ctx.i32_at(AppContext::LINEUP_BASE_LEVEL)? != -1 {
