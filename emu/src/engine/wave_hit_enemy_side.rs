...
use super::{AppContext, Entity, WaveRecord, WaveSprite, enemy_attack_dispatch, get_wave_block};

pub fn wave_hit_enemy_side(
    ctx: &mut AppContext,
    wave_index: i32,
    target: i32,
    attack: i32,
    ...
    )? && get_wave_block(ctx, 0, target)?
    {
        ...
        ctx.set_i32_at(
            AppContext::entity_field(0, target, Entity::WAVE_BLOCK_VFX_FRAME),
            0,
        )?;
        ctx.set_i32_at(
            AppContext::entity_field(0, target, Entity::WAVE_BLOCK_VFX_ACTIVE),
            1,
        )?;
    }
