...
use super::{
    abs_i32, call_rng, enemy_attack_dispatch, get_explosion_anchor, get_explosion_chance, get_explosion_span, 
    get_mini_surge, get_pos_x, get_surge_anchor, get_surge_chance, get_surge_level, get_surge_span, get_wave_chance, get_wave_level,
...
pub fn enemy_hit_executor(ctx: &mut AppContext, slot: i32, targets: &[i32], attack: i32) -> Result<(), Fault> {
    ...
    while index < targets.len() {
        ...
        if enemy_attack_dispatch(
            ...
            ctx.set_i32_at(AppContext::entity_field(0, target, Entity::HIT_SPARK_TYPE), long_range as i32)?;
        }
    ...
    if get_surge_chance(ctx, 1, slot)? > 0 {
        ...
        if get_surge_chance(ctx, 1, slot)? > roll {
            ...
            let offset = if get_surge_span(ctx, 1, slot)? > 0 { reach } else { reach.wrapping_neg() };
    ...
    if get_explosion_chance(ctx, 1, slot)? > 0 {
        ...
        if get_explosion_chance(ctx, 1, slot)? > roll {
            ...
            let offset = if get_explosion_span(ctx, 1, slot)? > 0 { reach } else { reach.wrapping_neg() };
