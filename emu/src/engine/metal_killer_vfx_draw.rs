...
pub fn metal_killer_vfx_draw(ctx: &mut AppContext) -> Result<(), Fault> {
    if ctx.metal_killer_vfx.is_empty() {
        return Ok(());
    }
    ...
    loop {
        maanim_execute(
            &mut ctx.metal_strong_model,
            Some(&ctx.metal_killer_vfx_anim),
            ctx.metal_killer_vfx[index].frame,
            0,
        )?;

        let pos_x = operation::div_10(
            ctx.metal_killer_vfx[index]
                .pos_x
        ...
        let x = operation::div_2(get_drawable_width(ctx)?.wrapping_add(-0x3c0))
            .wrapping_add(pos_x)
            .wrapping_add(0x44);
        let y = operation::div_10(ctx.metal_killer_vfx[index].pos_y).wrapping_add(0x4d);
