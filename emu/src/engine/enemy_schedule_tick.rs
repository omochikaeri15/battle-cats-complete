...
use super::{
    ...
    get_castle_enemy_row, get_entity_base_idx, get_hp, get_total_damage_taken, has_castle_enemy,
    read_flag, spawn_entity, stage_entry_base_trigger, stage_entry_count, stage_entry_death_gate,
    stage_entry_enemy_id, stage_entry_keep_delay, stage_entry_respawn_max, stage_entry_respawn_min,
...
pub fn enemy_schedule_tick(ctx: &mut AppContext) -> Result<(), Fault> {
    ...
    while row < ctx.stage_enemies.len() {
        ...
        if has_castle_enemy(ctx)?
            && ctx.i32_at(
                ((get_castle_enemy_row(ctx)? as i64) * ENEMY_STATS_STRIDE as i64
                    + (ENEMY_STATS + EnemyStats::TRAIT_DOJO) as i64) as usize,
            )? != 0
        {
    ...
    for slot in 1..0x33 {
        alive = alive.wrapping_add(
            (ctx.i32_at(AppContext::entity_field(1, slot, Entity::OCCUPANT))? != 0) as i32,
        );
    }
    ...
    if spawn_entity(
        ctx,
        1,
        stage_entry_row(&enemy_row),
        0,
        stage_entry_z_min(&enemy_row),
        stage_entry_z_max(&enemy_row),
        0,
        row,
    )? < 0
    {
    ...
    let spawn_span = ctx
        ...
        .wrapping_sub(spawn_min)
        .wrapping_add(1);
    let next_spawn =
        call_rng(ctx, spawn_span).wrapping_add(ctx.i32_at(AppContext::STAGE_SPAWN_MIN)?);

    ctx.set_i32_at(AppContext::SPAWN_COUNTDOWN, next_spawn)?;

    let respawn_span = stage_entry_respawn_max(&enemy_row)
        .wrapping_sub(stage_entry_respawn_min(&enemy_row))
        .wrapping_add(1);
    ...
    let seen =
        ((stage_entry_enemy_id(&enemy_row) as i64) * 4 + AppContext::SEEN_ENEMIES as i64) as usize;
