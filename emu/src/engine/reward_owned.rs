...
use super::{reward_unit_id, AppContext};
...
pub fn reward_owned(ctx: &AppContext, id: i32) -> Result<bool, Fault> {
    ...
    if (id.wrapping_sub(0x2710) as u32) <= 0x270f {
        ...
        return Ok(ctx.i32_at(((unit as i64) * 4 + AppContext::REWARD_UNITS_OWNED as i64) as usize)? != 0);
    }
    ...
    if id >= 0x4e20 {
        ...
        return Ok(ctx.i32_at(((unit as i64) * 4 + AppContext::REWARD_FORMS_OWNED as i64) as usize)? != 0);
    }
    ...
    for row in ctx.event_unit_rows.iter().take(count as u32 as usize) {
        if *row.first().ok_or(Fault::IndexOutOfRange { site: SITE, index: 0, limit: 0 })? == id {
            let slot = *row.get(1).ok_or(Fault::IndexOutOfRange { site: SITE, index: 1, limit: row.len() as i64 })?;

            if slot == -1 {
                return Ok(false);
            }

            return Ok(ctx.i32_at(((slot as i64) * 4 + AppContext::EVENT_UNIT_OWNED as i64) as usize)? != 0);
        }
    }
