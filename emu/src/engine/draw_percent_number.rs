...
use super::{
    ...
    imgcut_get_sprite_cut,
};

pub fn draw_percent_number(
    ctx: &mut AppContext,
    sheet: &Imgcut,
    digits: &Imgcut,
    x: i32,
    ...
    let across = ops::div_100(percent.wrapping_mul(-0x71))
        .wrapping_add(x)
        .wrapping_add(0x9c);
    let rise = ops::div_100(percent.wrapping_mul(0xd8));
    let down = rise.wrapping_add(y);
    let gauge = *imgcut_get_sprite_cut(sheet, 9)?;
    let cut_x = gauge[0];
    let cut_y = gauge[1];
    let cut_w = ops::div_100(0x64i32.wrapping_sub(percent).wrapping_mul(-0x71))
        .wrapping_add(gauge[2]);
    let cut_h = gauge[3];
    ...
    draw_cut(
        draw_context(&mut ctx.draw)?,
        sheet,
        edge.wrapping_add(0x2a),
        y.wrapping_add(rise).wrapping_add(-0x1c),
        1,
    );

    let places = digit_count(percent);
    let half = ops::div_2(imgcut_get_sprite_cut(sheet, 1)?[2]);
    ...
    let centre = ops::div_2(span.wrapping_add(mark).wrapping_add(1))
        .wrapping_add(start)
    ...
    draw_cut(
        ...
        sheet,
        centre,
        y.wrapping_add(rise).wrapping_add(-1),
        0x10,
    );
