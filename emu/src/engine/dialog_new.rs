...
pub trait UiHost {
    fn dialog_create(&mut self, dialog: u64, kind: i32, text: &[u8], first_button: i32, second_button: i32, flags: i32);
    ...
    fn page_list_layout(&mut self, list: usize, count: i32, rows: i32, center: f32, spacing: f32, width: f32);
}

pub fn dialog_new(ctx: &mut AppContext, kind: i32, text: &[u8], first_button: i32, second_button: i32, flags: i32, on_event: Option<DialogEventHandler>) -> Result<u64, Fault> {
    ...
    ctx.dialogs.objects.insert(
        dialog,
        Dialog { kind, text: text.to_vec(), first_button, second_button, flags, state: 0, button: -1, back_button: -3, on_event, on_draw: None, on_update: None },
    );

    ctx.ui().ok_or(Fault::HostMissing { site: "dialog_new" })?.dialog_create(dialog, kind, text, first_button, second_button, flags);
