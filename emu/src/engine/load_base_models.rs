...
use super::{
    asset_stream_open, get_castle_row, get_map_type, is_space_map, maanim_load, mamodel_load, mamodel_set_sheet_table, query_localizable, read_flag, string_format_int, string_format_int2, string_format_int3,
    texture_cache_load, AppContext,
};

const SITE: &str = "load_base_models";

pub fn load_base_models(ctx: &mut AppContext, faction: i32, cannon: i32, foundation: i32, decor: i32) -> Result<(), Fault> {
    if read_flag(ctx, AppContext::faction_flags(faction))? & 1 != 0 {
        ...
        for part in 0..4 {
            if part == 1 {
                ...
                let cut = string_format_int3(ctx, b"nyankoCastle_%03d_%02d_%02d.imgcut", 1, value, 0)?;

                let sheet = texture_cache_load(ctx, &png, &cut, 0x2601)?;

                ctx.base_sheets.get(1).ok_or(Fault::IndexOutOfRange { site: SITE, index: 1, limit: ctx.base_sheets.len() as i64 })?.set(sheet);

                let mut variant = 0;

                loop {
                    let name = string_format_int3(ctx, b"nyankoCastle_%03d_%02d_%02d.mamodel", 1, value, variant)?;
                    ...
                    let mut model = std::mem::take(ctx.base_models.get_mut(slot).ok_or(Fault::IndexOutOfRange { site: SITE, index: slot as i64, limit })?);

                    mamodel_load(ctx, &mut model, &name)?;
                    ctx.base_models[slot] = model;

                    let name = string_format_int3(ctx, b"nyankoCastle_%03d_%02d_%02d.maanim", 1, value, variant)?;
                    let limit = ctx.base_anims.len() as i64;
                    let mut anim = std::mem::take(ctx.base_anims.get_mut(slot).ok_or(Fault::IndexOutOfRange { site: SITE, index: slot as i64, limit })?);
                ...
                ctx.base_sheets.get(part as usize).ok_or(Fault::IndexOutOfRange { site: SITE, index: part as i64, limit: ctx.base_sheets.len() as i64 })?.set(sheet);
            }
        }
    ...
    let sheet = if get_map_type(ctx, 0)? == -2 || get_map_type(ctx, 0)? == -12 {
        ...
        let png = query_localizable(ctx, &name);
        let variant = get_castle_row(&ctx.enemy_castle, ctx.i32_at(AppContext::CASTLE_ID)?)?.art_variant;
