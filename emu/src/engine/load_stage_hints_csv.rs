...
pub fn load_stage_hints_csv(ctx: &mut AppContext) -> Result<(), Fault> {
    ctx.fixed_lineup_store.hints.clear();

    let Some(bytes) = open_asset_stream(ctx, b"stage_hint_popup.csv", 0, 0)? else {
        return Ok(());
    };
    ...
    while read_csv_row(stm) {
        ctx.fixed_lineup_store.hints.push(FixedLineupRow::default());

        let value = read_csv_cell(stm, 0) as i32;

        if let Some(row) = ctx.fixed_lineup_store.hints.last_mut() {
            row.map_id = value;
        }
