...
use super::{map_type_base_id, AppContext};

const SITE: &str = "set_stage_unlock";

pub fn set_stage_unlock(ctx: &mut AppContext, map_type: i32, map_idx: i32, star: i32, value: i32, use_cache: i32) -> Result<(), Fault> {
    if use_cache != 0 {
    ...
    match case {
        ...
        0x06..=0x0a | 0x0f => {
            let (maps, cell) = match case {
                0x06 => (&mut ctx.stage_unlock_neg20, (map_idx as i64) * 4 + star as i64),
                0x07 => (&mut ctx.stage_unlock_neg19, map_idx as i64 + star as i64),
                0x08 => (&mut ctx.stage_unlock_neg18, (map_idx as i64) * 4 + star as i64),
                0x09 => (&mut ctx.stage_unlock_neg17, map_idx as i64 + star as i64),
                0x0a => (&mut ctx.stage_unlock_neg16, (map_idx as i64) * 4 + star as i64),
                _ => (&mut ctx.stage_unlock_neg11, (map_idx as i64) * 4 + star as i64),
            };
            let limit = maps.len() as i64;
