...
pub fn sniper_update(ctx: &mut AppContext) -> Result<(), Fault> {
    ...
    ctx.set_i32_at(AppContext::SNIPER_LOCKED, 0)?;
    ...
    if get_battle_status(ctx)? == 0 {
        ...
        while slot != 51 {
            'slot: {
                ...
                if x < ctx.i32_at(AppContext::SNIPER_LOCKED)? {
                    break 'slot;
                }

                ctx.set_i32_at(AppContext::SNIPER_LOCKED, x)?;
        ...
        ctx.set_i32_at(AppContext::EFFECT_ORIGIN_X, gun_x)?;
        ...
        ctx.set_i32_at(AppContext::EFFECT_ORIGIN_Y, gun_y)?;
    }
    ...
    if target > 0 {
        ...
        let rise = (operation::div_neg_10(pos_y as i64) as i32).wrapping_add(ctx.i32_at(AppContext::EFFECT_ORIGIN_Y)?).wrapping_sub(z_layer.wrapping_shl(2)).wrapping_add(0x3a) as f32;
        let pos_x = ctx.i32_at(AppContext::entity_field(1, target, Entity::POS_X))?.wrapping_sub(ctx.i32_at(AppContext::CAMERA_X)?);
        let run = (operation::div_neg_10(pos_x as i64) as i32).wrapping_add(ctx.i32_at(AppContext::EFFECT_ORIGIN_X)?) as f32;
    ...
    if arrived {
        ...
        ctx.set_i32_at(AppContext::SNIPER_LOCKED, 1)?;
    }
    ...
    ctx.set_i32_at(AppContext::EFFECT_ORIGIN_X, muzzle_x)?;
    ...
    ctx.set_i32_at(AppContext::EFFECT_ORIGIN_Y, muzzle_y)?;
    ...
    'sequence: {
        if ctx.i32_at(AppContext::SNIPER_LOCKED)? == 1 && charge >= 0x12c {
            if firing == 0 {
        ...
        if frame == 0xb {
            ...
            let origin_x = ctx.i32_at(AppContext::EFFECT_ORIGIN_X)?;

            ctx.set_i32_at(AppContext::PENDING_STRIKE_TRIGGER_X.wrapping_add(free.wrapping_mul(4)), origin_x)?;

            let origin_y = ctx.i32_at(AppContext::EFFECT_ORIGIN_Y)?;
