...
pub fn sniper_update(ctx: &mut AppContext) -> Result<(), Fault> {
    ...
    if get_battle_status(ctx)? == 0 {
        ...
        while slot != 51 {
            'slot: {
                ...
                if x < ctx.i32_at(AppContext::DRAW_TEMP_0)? {
                    break 'slot;
                }

                ctx.set_i32_at(AppContext::DRAW_TEMP_0, x)?;
        ...
        ctx.set_i32_at(AppContext::DRAW_TEMP_2, gun_y)?;
    }
    ...
    ctx.set_i32_at(AppContext::DRAW_TEMP_2, muzzle_y)?;

    let charge = ctx.i32_at(AppContext::SNIPER_CHARGE)?;
    let charge = if charge < 0x12b {
        charge.wrapping_add(1)
    } else {
        0x12c
    };
    ...
    'sequence: {
        if ctx.i32_at(AppContext::DRAW_TEMP_0)? == 1 && charge >= 0x12c {
            if firing == 0 {
        ...
        if frame == 0xb {
            ...
            while free != 50
                && ctx.u8_at(AppContext::PENDING_STRIKE_ACTIVE.wrapping_add(free))? != 0
            {
                free += 1;
            }
            ...
            ctx.set_i32_at(
                AppContext::PENDING_STRIKE_SPEED.wrapping_add(free.wrapping_mul(4)),
                0x5dc,
            )?;

            let origin_x = ctx.i32_at(AppContext::DRAW_TEMP_1)?;

            ctx.set_i32_at(
                AppContext::PENDING_STRIKE_TRIGGER_X.wrapping_add(free.wrapping_mul(4)),
                origin_x,
            )?;

            let origin_y = ctx.i32_at(AppContext::DRAW_TEMP_2)?;

            ctx.set_i32_at(
                AppContext::PENDING_STRIKE_Y.wrapping_add(free.wrapping_mul(4)),
                origin_y,
            )?;
            ctx.set_i32_at(
            ...
            let seeded = [
                0i32, 0, 0, 0x1e, 0, 0, 0, 0, -0x1e, 0, 0, 0, 0, 0, 0x1e, 0, 0, 0, 0, -0x1e,
            ];
            let mut cell = 0usize;
