...
use super::{refresh_cat_speeds, get_global_map_id, get_special_rule_params, play_sound, sound_manager, AppContext};

const SITE: &str = "fever_tick";

pub fn fever_tick(ctx: &mut AppContext, points: i32) -> Result<(), Fault> {
    let map_id = get_global_map_id(ctx, 0)?;
    let Some(params) = get_special_rule_params(ctx, &ctx.special_rules, map_id, 0xc)?.cloned() else {
        return Ok(());
    };
    ...
    let fever_rule = *params.first().ok_or(Fault::IndexOutOfRange { site: SITE, index: 0, limit: params.len() as i64 })?;

    if fever_rule != 0 {
        return Ok(());
    }

    let needed = *params.get(1).ok_or(Fault::IndexOutOfRange { site: SITE, index: 1, limit: params.len() as i64 })?;
    ...
    let step = if mid.wrapping_sub(fill) >= 2 { mid } else { fill.wrapping_add(1) };
    ...
    let length = *params.get(2).ok_or(Fault::IndexOutOfRange { site: SITE, index: 2, limit: params.len() as i64 })?;
