...
pub fn get_cannon_charge_frames(ctx: &mut AppContext) -> Result<i32, Fault> {
    ...
    let chapter = if ctx.i32_at(AppContext::CHAPTER_MODE)? > 2 { 0x546 } else { ctx.i32_at(AppContext::CHAPTER_MODE)?.wrapping_mul(0x1c2) };
    let reduced = upgrade.wrapping_mul(0x32).wrapping_sub(treasure);
    let reduced = if reduced > 0 { reduced } else { 0 };
    let frames = get_base_upgrade(ctx, 3)?.wrapping_mul(-0x32).wrapping_add(chapter).wrapping_add(reduced);
    let frames = frames.wrapping_sub(get_treasure_value(ctx, &ctx.treasure_store, 1)?).wrapping_add(0x5dc);
