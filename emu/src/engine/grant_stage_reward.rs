...
use super::{
    add_resource, analytics_params, event_unit_slot_by_item, find_item_index, get_global_map_id, get_powerup, get_stage_index, get_star_level, level_cell_add,
    level_cell_base, mission_progress, orb_inventory_add, reward_unit_id, unit_buy_field, xor_row46_get, AppContext, FormatArg, UNIT_BUY, UNIT_BUY_STRIDE,
};
...
pub fn grant_stage_reward(ctx: &mut AppContext, field: i32, first: u8) -> Result<i32, Fault> {
    ...
    let item = xor_row46_get(ctx.bytes_from(row)?, index).ok_or(Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: 0x2e })? as i32;
    let kind = find_item_index(ctx, item)?;
    let item = xor_row46_get(ctx.bytes_from(row)?, index).ok_or(Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: 0x2e })? as i32;

    if kind != -1 {
        let kind = find_item_index(ctx, item)?;
        let amount = xor_row46_get(ctx.bytes_from(row)?, index + 1).ok_or(Fault::IndexOutOfRange { site: SITE, index: index as i64 + 1, limit: 0x2e })? as i32;
    ...
    'owned: {
            ...
            if item > ctx.drop_chara_max_1000 {
                ...
                if item as u32 >= 0x44c {
                    if item <= ctx.drop_chara_max_1100 {
                        ...
                        let value = xor_row46_get(ctx.bytes_from(row)?, index).ok_or(Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: 0x2e })? as i32;
        ...
        if value >= bound && xor_row46_get(ctx.bytes_from(row)?, index).ok_or(Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: 0x2e })? as i32 <= limit {
            break 'owned;
        }
        ...
        let result = 3 + (level_cell_base(ctx, cell)? >= unit_buy_field(ctx.bytes_from(buy)?, 0x13)?.wrapping_sub(1)) as i32;
