...
use super::{
    connecting_indicator_hide, dialog_show_alt, play_sound, query_localizable, ranking_record_rank, request_save_data, scored_map_pays_money, sound_manager,
    trial_win_update_lambda_0_0, AppContext, RankingRecord,
};

const SITE: &str = "trial_win_update_lambda_0";

pub fn trial_win_update_lambda_0(ctx: &mut AppContext, status: i32, record: Option<&RankingRecord>) -> Result<(), Fault> {
    ...
    match status {
        ...
        0 => {
            if ranking_record_rank(record.ok_or(Fault::NullPointer { site: SITE })?) <= 0x63 {
                let sound = if scored_map_pays_money(ctx)? { 0xbe } else { 0x40 };
