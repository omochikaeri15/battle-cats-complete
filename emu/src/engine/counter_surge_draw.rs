...
use super::{draw_context, draw_model, get_drawable_width, maanim_execute, AppContext, Entity};
...
pub fn counter_surge_draw(ctx: &mut AppContext, faction: i32, slot: i32) -> Result<(), Fault> {
    ...
    while index < ctx.counter_surge_events.len() {
        ...
        let event = ctx.counter_surge_events.get(index).ok_or(Fault::OutOfRange { site: SITE })?;
        ...
        let model = if faction == 0 { &ctx.demonsummon_model } else { &ctx.demonsummon_e_model };
