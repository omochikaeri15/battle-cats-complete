...
pub fn cat_god_menu_input(ctx: &mut AppContext) -> Result<bool, Fault> {
    ...
    match state {
        0 => {
            ...
            ctx.set_f32_at(
                ...
                ctx.f32_at(AppContext::CAT_GOD_SPIN)? + speed,
            )?;
            ctx.set_i32_at(
                AppContext::CAT_GOD_BUTTON_SINK,
                ctx.i32_at(AppContext::CAT_GOD_BUTTON_SINK)?.wrapping_sub(1),
            )?;
        ...
        1 => {
            ...
            let drop = ctx
                .i32_at(AppContext::CAT_GOD_DROP_SPEED)?
                .wrapping_add(0x28);

            ctx.set_i32_at(AppContext::CAT_GOD_DROP_SPEED, drop)?;
            ctx.set_i32_at(
                AppContext::CAT_GOD_BUTTON_SINK,
                ctx.i32_at(AppContext::CAT_GOD_BUTTON_SINK)?
                    .wrapping_add(drop),
            )?;
        ...
        3 => {
            ...
            if step == 0 {
                ...
                ctx.set_i32_at(
                    AppContext::CAT_GOD_OPEN_TICKS,
                    ctx.i32_at(AppContext::CAT_GOD_OPEN_TICKS)?.wrapping_add(1),
                )?;
                chatter = true;
                ...
                if let Some((miracle, presses)) = picked {
                    if presses as u32 >= 5 {
                        ...
                        if step as u32 >= 4 {
                            ...
                            if (obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_16_COUNT)?)
                                as i32)
                                < price
                            {
                                ctx.set_i32_at(AppContext::CAT_GOD_CHATTER_TIMER, 0x12c)?;
                    ...
                    if presses as u32 >= 5 {
                        ...
                        let food =
                            obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_16_COUNT)?) as i32;
                    ...
                    if ctx.u8_at(AppContext::CAT_GOD_CONFIRM_OPEN)? == 0 {
                        if step as u32 >= 4 {
                            for miracle in 0..3usize {
                                ...
                                if !held {
                                    ctx.set_block_at::<1>(
                                        AppContext::CAT_GOD_HOVER + miracle,
                                        [0],
                                    )?;
                                ...
                                for miracle in 0..3usize {
                                    ...
                                    if pressed {
                                        ...
                                        ctx.set_i32_at(
                                            ...
                                            ctx.i32_at(AppContext::CAT_GOD_PRESSES + miracle * 4)?
                                                .wrapping_add(1),
                                        )?;
                                    }
                                }
                        ...
                        if confirmed {
                            let sound = if ctx.i32_at(AppContext::CAT_GOD_INTRO_STEP)? < 4 {
                                ...
                                let price = get_miracle_price(
                                    ctx,
                                    ctx.i32_at(AppContext::CAT_GOD_SELECTED)?,
                                )?;
                                let food =
                                    obf_value_read(&ctx.block_at::<8>(AppContext::ITEM_16_COUNT)?)
                                        as i32;
        ...
        4 => {
            ...
            if frame == 0 {
                ...
                ctx.set_i32_at(
                    AppContext::CAT_GOD_OFFSET,
                    ctx.i32_at(AppContext::CAT_GOD_OFFSET)?
                ...
                if frame as u32 > 0x26 {
                    ...
                    ctx.set_i32_at(
                        AppContext::CAT_GOD_OFFSET,
                        ctx.i32_at(AppContext::CAT_GOD_OFFSET)?.wrapping_sub(1),
                    )?;
                }
    ...
    if chatter {
        let due = if ctx.u8_at(AppContext::CAT_GOD_CONFIRM_OPEN)? == 0
            || ctx.i32_at(AppContext::CAT_GOD_INTRO_STEP)? < 4
        {
    ...
    if ctx.i32_at(AppContext::CAT_GOD_STATE)? & !1 == 4
        && ctx.i32_at(AppContext::CAT_GOD_ZOOM_DONE)? == 0
    {
        ...
        let (delta, min_zoom) = if ticks == 0 {
            ...
            (
                ctx.i32_at(AppContext::CAT_GOD_ZOOM_DELTA)?,
    ...
    if ctx.i32_at(AppContext::CAT_GOD_STATE)? == 5 {
        ...
        if !reached {
            ctx.set_i32_at(
                AppContext::CAT_GOD_TICKS,
                ctx.i32_at(AppContext::CAT_GOD_TICKS)?.wrapping_add(1),
            )?;
            ...
            1 => {
                ...
                if old == 0 {
                    ...
                    for slot in 1..0x33 {
                        ...
                        {
                            ctx.set_i32_at(
                                AppContext::entity_field(1, slot, Entity::GOD_PUSH_FRAME),
                                0,
                            )?;
                            ctx.set_i32_at(
                                AppContext::entity_field(1, slot, Entity::GOD_PUSH_STEP),
                                10,
                            )?;
                            ...
                            ctx.set_i32_at(
                                AppContext::entity_field(1, slot, Entity::GOD_PUSH_DELAY),
                                delay,
                            )?;
                        }
                    }
                ...
                if frames > 0 {
                    for slot in 1..0x33 {
                        if ctx.i32_at(AppContext::entity_field(1, slot, Entity::OCCUPANT))? != 0
                            ...
                            && get_entity_state(ctx, 1, slot)? != 0x15
                        {
                            let pushed = ctx.i32_at(AppContext::entity_field(
                                1,
                                slot,
                                Entity::GOD_PUSH_FRAME,
                            ))?;
                            let delay = ctx.i32_at(AppContext::entity_field(
                                1,
                                slot,
                                Entity::GOD_PUSH_DELAY,
                            ))?;
                            let movable = can_push_back(ctx, 1, slot)?;

                            if pushed >= delay {
                                if movable {
                                    let step = ctx.i32_at(AppContext::entity_field(
                                        1,
                                        slot,
                                        Entity::GOD_PUSH_STEP,
                                    ))?;
            ...
            2 => {
                ...
                for wave in 0..9usize {
                    ...
                    loop {
                        ...
                        if ctx.i32_at(AppContext::CAT_GOD_SHAKE_X + wave * 4)? != x
                            || ctx.i32_at(AppContext::CAT_GOD_SHAKE_Y + wave * 4)? != y
                        {
                            break;
                        }
                    }
        ...
        if finished {
            ...
            set_bgm_duck(
                sound_manager(ctx)?,
                100i32.wrapping_sub(ticks.wrapping_mul(10)),
            );
            ...
            if ctx.i32_at(AppContext::CAT_GOD_FADE)? <= 0
                && ctx.i32_at(AppContext::CAMERA_ZOOM)?
                    >= ctx.i32_at(AppContext::CAT_GOD_SAVED_ZOOM)?
            {
                if ctx.i32_at(AppContext::CAT_GOD_SELECTED)? == 3 {
