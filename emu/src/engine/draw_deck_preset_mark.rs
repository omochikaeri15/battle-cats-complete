...
pub fn draw_deck_preset_mark(
    ...
    let cell =
        ctx.block_at::<4>(AppContext::BATTLE_DECK.wrapping_add(((slot as i64) * 4) as usize))?;
    ...
    let base = ((ctx.i32_at(AppContext::SELECTED_DECK_PRESET)? as i64)
        * AppContext::DECK_PRESET_STRIDE as i64) as usize;
    let key = ctx.block_at::<4>(base.wrapping_add(AppContext::DECK_PRESET_KEY))?;
    let cell = ctx.block_at::<4>(
        base.wrapping_add(AppContext::DECK_PRESETS)
            .wrapping_add(((slot as i64) * 4) as usize),
    )?;
    let saved = i32::from_le_bytes([
        key[0] ^ cell[0],
        key[1] ^ cell[1],
        key[2] ^ cell[2],
        key[3] ^ cell[3],
