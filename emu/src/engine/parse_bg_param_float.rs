...
use super::{
    bg_param_base_id, json_container_as_double, json_container_as_int, json_container_as_string, json_string_as_double, json_string_as_int, json_string_as_string, json_value_as_double, json_value_as_int, json_value_as_string, parse_bg_param_bound_float, BgParamSpec, JsonNode,
};

const SITE: &str = "parse_bg_param_float";

pub fn parse_bg_param_float(spec: &mut BgParamSpec<f32>, node: Option<&BTreeMap<Vec<u8>, JsonNode>>) -> Result<(), Fault> {
    spec.enabled = 0;
    spec.value = 0.0;
    spec.values.clear();
    spec.base = 0;
    ...
    if let Some(found) = node.get(b"value".as_slice()) {
        ...
        if let Some(JsonNode::Object(bound)) = node.get(b"min".as_slice()) {
            parse_bg_param_bound_float(&mut spec.has_min, &mut spec.min, &mut spec.min_base, bound)?;
        }

        if let Some(JsonNode::Object(bound)) = node.get(b"max".as_slice()) {
            parse_bg_param_bound_float(&mut spec.has_max, &mut spec.max, &mut spec.max_base, bound)?;
        }
