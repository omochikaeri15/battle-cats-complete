...
pub fn surge_update(ctx: &mut AppContext) -> Result<(), Fault> {
    ...
    while (index as i64 as usize) < ctx.surge_events.len() {
        ...
        let missing = Fault::IndexOutOfRange { site: SITE, index: index as i64, limit: ctx.surge_events.len() as i64 };
        ...
        let tick = operation::idiv(elapsed, SURGE_TIMING[2]).ok_or(Fault::divide(SITE, SURGE_TIMING[2] as i64))?;
        ...
        while slot != 51 {
            ...
            if hit_ticks.contains_key(&slot) && *hit_ticks.get(&slot).ok_or(Fault::KeyNotFound { site: SITE, key: slot as i64 })? == tick {
                slot += 1;
            ...
            let x = if faction == 1 {
                get_pos_x(ctx, 0, slot)?.wrapping_add(ctx.i32_at(AppContext::entity_field(0, slot, Entity::HITBOX_POS))?).wrapping_sub(SURGE_TIMING[4])
            } else {
                get_pos_x(ctx, other, slot)?.wrapping_sub(ctx.i32_at(AppContext::entity_field(other, slot, Entity::HITBOX_POS))?).wrapping_add(SURGE_TIMING[4])
            };
            ...
            *ctx.surge_events.get_mut(at).ok_or(missing.clone())?.hit_ticks.entry(slot).or_insert(0) = tick;
            slot += 1;
        }
