...
use super::{
    AppContext, aku_realm_final_redirect, ex_redirect_check_c, ex_replacement_pending,
    get_ex_option_target, get_global_map_id, get_map_type, get_stage_index, get_crown_level,
    is_ex_option_target, map_type_base_id,
};

pub fn get_built_deck_stage_key(ctx: &mut AppContext) -> Result<i32, Fault> {
    let map_id = get_global_map_id(ctx, 0)?;
    let stage = get_stage_index(ctx)?;
    let crown_base = if get_crown_level(ctx)? != 3 {
        0
    } else {
        1_000_000_000i32
    };
    ...
    if ex_replacement_pending(ctx, -1, -1)? {
        return Ok(crown_base
            .wrapping_add(stage)
            .wrapping_add(get_ex_option_target(ctx, map_id).wrapping_mul(1000)));
    }

    if is_ex_option_target(ctx)? {
        return Ok(stage
            .wrapping_add(crown_base)
