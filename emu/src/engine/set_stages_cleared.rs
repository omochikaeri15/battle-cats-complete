...
use super::{map_type_base_id, set_cleared_count, AppContext};

const SITE: &str = "set_stages_cleared";

pub fn set_stages_cleared(ctx: &mut AppContext, map_type: i32, map_idx: i32, star: i32, value: i32, use_cache: i32) -> Result<(), Fault> {
    if use_cache != 0 {
        let map_id = map_type_base_id(map_type, map_idx);
        let stars = ctx.stages_cleared_cache.entry(map_id).or_default();
        let cell = stars.get_mut(star as i64 as usize).ok_or(Fault::IndexOutOfRange { site: SITE, index: star as i64, limit: 4 })?;
    ...
    match case {
        ...
        0x06 | 0x08..=0x0a | 0x0f => {
            let (maps, cell) = match case {
                0x06 => (&mut ctx.stages_cleared_neg20, (map_idx as i64) * 4 + star as i64),
                0x08 => (&mut ctx.stages_cleared_neg18, (map_idx as i64) * 4 + star as i64),
                0x09 => (&mut ctx.stages_cleared_neg17, map_idx as i64 + star as i64),
                0x0a => (&mut ctx.stages_cleared_neg16, (map_idx as i64) * 4 + star as i64),
                _ => (&mut ctx.stages_cleared_neg11, (map_idx as i64) * 4 + star as i64),
            };
            let limit = maps.len() as i64;
            let slot = maps.get_mut(cell as usize).ok_or(Fault::IndexOutOfRange { site: SITE, index: cell, limit })?;
