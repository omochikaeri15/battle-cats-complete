...
use super::{
    add_entity_frame, get_anim_len, get_entity_button, get_entity_frame, get_entity_state, get_spawn_anim_flag,
...
pub fn advance_animation_frame(ctx: &mut AppContext, faction: i32, slot: i32) -> Result<(), Fault> {
    ...
    if get_entity_state(ctx, faction, slot)? == 1 {
        let anim = get_unit_anim(ctx, faction, button, 1)?.ok_or(Fault::NullPointer { site: SITE })?;
    ...
    if get_entity_state(ctx, faction, slot)? == 2 {
        let anim = get_unit_anim(ctx, faction, button, 2)?.ok_or(Fault::NullPointer { site: SITE })?;
        let length = get_anim_len(anim)?;

        if faction == 0 && length == -1 {
            let next = get_entity_frame(ctx, 0, slot)?.wrapping_add(1);
            let anim = get_unit_anim(ctx, 0, button, 2)?.ok_or(Fault::NullPointer { site: SITE })?;
    ...
    if get_entity_state(ctx, faction, slot)? == 3 {
        ...
        let frame = current.wrapping_add(cycles.wrapping_shl(3).wrapping_mul(3).wrapping_neg()).wrapping_add(1);
    ...
    if get_entity_state(ctx, faction, slot)? == 5 || get_entity_state(ctx, faction, slot)? == 6 {
        ...
        let frame = current.wrapping_add(cycles.wrapping_shl(2).wrapping_mul(3).wrapping_neg()).wrapping_add(1);
    ...
    if get_entity_state(ctx, faction, slot)? == 7 {
        ...
        let frame = current.wrapping_add(cycles.wrapping_shl(4).wrapping_mul(3).wrapping_neg()).wrapping_add(1);
    ...
    if get_entity_state(ctx, faction, slot)? == 0xa {
        ...
        if get_spawn_anim_flag(ctx, faction, slot)? {
            let anim = get_unit_anim(ctx, faction, button, 7)?.ok_or(Fault::NullPointer { site: SITE })?;
        ...
        if get_spawn_anim_type(ctx, faction, slot)? >= 0 {
            ...
            let effect_length = get_anim_len(std_map_int_maanim_subscript_2(&mut ctx.effect_anims, &key))?;
