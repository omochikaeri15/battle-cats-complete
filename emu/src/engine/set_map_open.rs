    ...
    match case {
        ...
        0x06 | 0x08..=0x0a | 0x0f => {
            let (maps, cell) = match case {
                0x06 => (&mut ctx.map_open_neg20, (map_idx as i64) * 4 + crown as i64),
                0x08 => (&mut ctx.map_open_neg18, (map_idx as i64) * 4 + crown as i64),
                0x09 => (&mut ctx.map_open_neg17, map_idx as i64 + crown as i64),
                0x0a => (&mut ctx.map_open_neg16, (map_idx as i64) * 4 + crown as i64),
                _ => (&mut ctx.map_open_neg11, (map_idx as i64) * 4 + crown as i64),
            };
            let limit = maps.len() as i64;
        ...
        0x14 => ctx.set_i32_at(
            ((map_idx as i64) * 0x10 + (crown as i64) * 4 + AppContext::MAP_OPEN_NEG6 as i64)
                as usize,
            value,
        ),
        _ => Ok(()),
