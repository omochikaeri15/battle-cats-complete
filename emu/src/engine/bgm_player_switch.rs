...
use super::{bgm_player_pick, get_global_map_id, play_sound, sound_manager, AppContext};
...
pub fn bgm_player_switch(ctx: &mut AppContext, boss: i32, keep_playing: u8) -> Result<(), Fault> {
    ...
    if keep_playing == 0 {
        ...
        let delay = ctx.map_records.range(map_id..).next().filter(|(key, _)| **key <= map_id).map_or(0, |(_, record)| record.bgm_delay);
