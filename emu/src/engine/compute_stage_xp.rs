...
use super::{get_base_upgrade, get_cat_combo_bonus, get_powerup, get_stage_record, get_treasure_value, min_i32, validate_map_type, xor_row46_get, AppContext};
...
pub fn compute_stage_xp(ctx: &mut AppContext) -> Result<i32, Fault> {
    ...
    if ctx.i32_at(AppContext::CHAPTER_MODE)? == 0x63 {
        xp = xor_row46_get(ctx.bytes_from(row)?, 1).ok_or(Fault::IndexOutOfRange { site: SITE, index: 1, limit: 0x2e })? as i32;
    } else {
        let map_type = validate_map_type(ctx.i32_at(AppContext::SAVED_MAP_TYPE)?);
        let legend_bonus = (map_type.wrapping_add(0x16) as u32) <= 0x16 && 0x402001u32 >> map_type.wrapping_add(0x16) & 1 != 0;

        xp = if ctx.i32_at(AppContext::CHAPTER_MODE)? != 3 {
            ctx.i32_at(AppContext::STAGE_ROW)?.wrapping_mul(0x1770).wrapping_add(0x4e20) / 10
        } else if legend_bonus {
            (xor_row46_get(ctx.bytes_from(row)?, 1).ok_or(Fault::IndexOutOfRange { site: SITE, index: 1, limit: 0x2e })? as i32).wrapping_mul(9)
        } else {
            xor_row46_get(ctx.bytes_from(row)?, 1).ok_or(Fault::IndexOutOfRange { site: SITE, index: 1, limit: 0x2e })? as i32
        };
        ...
        if chapter != 3 {
            ...
            let cleared = operation::xor_row_decode(records, 0x33, stage as i64 as usize).ok_or(Fault::IndexOutOfRange { site: SITE, index: stage as i64, limit: 0x33 })? as i32;
    ...
    let treasure = get_treasure_value(ctx, &ctx.treasure_store, 6)?.wrapping_add(upgrade).wrapping_add(0x64);
