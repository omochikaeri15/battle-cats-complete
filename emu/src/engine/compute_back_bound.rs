...
use super::{
    get_castle_id, get_castle_row, get_entity_button, get_unit_model, mamodel_get_anchor, mamodel_get_anchor_part,
    mamodel_get_anchor_x, mamodel_get_anchor_y, read_flag, transform_anchor, AppContext, Entity,
};

const SITE: &str = "compute_back_bound";
...
pub fn compute_back_bound(ctx: &mut AppContext, faction: i32, slot: i32) -> Result<(), Fault> {
    ...
    if faction == 0 {
        let bound = ctx.i32_at(AppContext::STAGE_LENGTH)?.wrapping_sub((reach << 2).wrapping_mul(5));

        return ctx.set_i32_at(AppContext::entity_field(faction, slot, Entity::BACK_BOUND), bound);
    }

    ctx.set_i32_at(AppContext::entity_field(faction, slot, Entity::BACK_BOUND), reach.wrapping_add(reach).wrapping_mul(5))?;
    ...
    let inset = (operation::div_neg_100(size.wrapping_mul(0x49c) as i64) as i32).wrapping_add(base_x);
    let offset_x = get_castle_row(&ctx.enemy_castle, get_castle_id(ctx)?)?.offset_x;
    let size = get_castle_row(&ctx.enemy_castle, get_castle_id(ctx)?)?.size;
    let shifted = (operation::div_10(size.wrapping_mul(offset_x) as i64) as i32).wrapping_add(inset);
    let size = get_castle_row(&ctx.enemy_castle, get_castle_id(ctx)?)?.size;
    ...
    ctx.set_i32_at(AppContext::entity_field(faction, slot, Entity::BACK_BOUND), bound)
}
