...
pub fn cat_attack_dispatch(
    ...
    if hp <= ops::div_100(get_strengthen_threshold(ctx, 0, attacker)?.wrapping_mul(max_hp)) {
        let max_hp = get_max_hp(ctx, 0, attacker)?;
    ...
    if target == 0 || get_entity_base_idx(ctx)? == target {
        ...
        if kaijin_combo > 0 && get_trait_kaijin(ctx, 1, target)? {
            damage = ops::div_1000(damage.wrapping_mul(kaijin_combo as u32 as i64));
        }
    ...
    if form >= 2 {
        ...
        if percent > 0 {
            ...
            damage = damage.wrapping_add(ops::div_100(base.wrapping_mul(percent)) as i64);
        }
    }
