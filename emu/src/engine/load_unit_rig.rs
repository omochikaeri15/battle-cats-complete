    ...
    for button in 0..buttons {
        ...
        if faction == 1 {
            if read_flag(ctx, AppContext::faction_flags(1))? & 1 != 0 {
                ...
                form = ctx.i32_at(
                    AppContext::FACTION_1_UNIT_FORMS
                        .wrapping_add((unit_row as i64 as usize).wrapping_mul(4)),
                )?;
            }
        ...
        let variant = if read_flag(ctx, AppContext::faction_flags(faction))? & 1 == 1 {
            form
        } else {
            -1
        };
        ...
        for index in 0..4 {
            ...
            *std_map_int_maanim_subscript(&mut ctx.unit_anims[side][button as usize], &index) =
                anim;
        }

        if stat_spawn_animation_flag(ctx, faction, unit, form)? {
            let path = format_localized(ctx, b"%s_entry.maanim", &base)?;
            let mut anim = std::mem::take(std_map_int_maanim_subscript(
                &mut ctx.unit_anims[side][button as usize],
                &7,
            ));
        ...
        if read_flag(ctx, AppContext::faction_flags(faction))? & 2 != 0
            && trait_zombie(ctx, faction, unit, 0, 1)?
        {
            for stage in 0..3 {
                let key = stage + 4;
                ...
                maanim_load(ctx, &mut anim, &path)?;
                *std_map_int_maanim_subscript(&mut ctx.unit_anims[side][button as usize], &key) =
                    anim;
            }
        }
