...
pub fn show(
    ...
    // Only use "Instant Load" if the app has absolutely NOTHING to show (Startup).
    // Otherwise, treat Unit Switch as a seamless transition.
    let is_app_startup = model_data.is_none() && anim_sheet.image_data.is_none();
    ...
    // Applies to BOTH Form Swaps AND Unit Swaps
    if !is_stable && !is_loading_new && !is_app_startup {
        ...
        // 1. Load Sheet into STAGING (Background)
        ...
        // 2. Load Model into STAGING (Background)
    ...
    // C. CHECK FOR TRANSITION COMPLETION
    if is_loading_new {
            ...
            if texture_is_ready {
                ...
                if let (Some(new_model), Some(new_sheet)) = (anim_viewer.staging_model.take(), anim_viewer.staging_sheet.take()) {
                    ...
                    *anim_sheet = new_sheet; 
                    ...
                    // FORCE REPAINT: Ensures unit switch is drawn instantly
                    ctx.request_repaint();
                }
            }
        }
    ...
    ui.vertical(|ui| {
        ...
        ui.put(rect, |ui: &mut egui::Ui| {
            if let Some(model) = model_data {
                ...
                // STABLE RENDERING:
                // We always render if we have data. 
                // The staging logic (allow_texture_update=false) protects us from glitches.
                // The request_repaint() ensures the new unit appears instantly.
                anim_viewer.render(
                    ui, anim_sheet, model,
