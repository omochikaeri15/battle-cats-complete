...
pub struct AnimViewer {
    ...
    // Helper to track if we've scanned the attack anim yet
...
impl AnimViewer {
    ...
    pub fn render(
        ...
        if self.loaded_id != self.last_loaded_id {
            ...
            // Reset scan state on unit change
        ...
        if self.is_selecting_export_region {
            ...
            // ... (tooltip code omitted for brevity, same as previous) ...
        ...
        // =====================================================================================
        // PHASE 0: PRE-SCAN ATTACK (For Showcase UI)
        // =====================================================================================
        // If popup is open, showcase is enabled, and we haven't scanned attack yet, find it.
        ...
        // Handle Showcase Animation Switching
        // =====================================================================================
        ...
        if self.export_state.is_processing && self.export_state.showcase_mode {
            ...
            let attack_dur = self.export_state.showcase_attack_len; // Can be 0 if skipped
            ...
            // Logic:
            // 0 -> walk_dur: Walk
            // walk_dur -> +idle_dur: Idle
            // +idle_dur -> +attack_dur: Attack
            // +attack_dur -> +kb_dur: KB
            
            if p < walk_dur {
                target_index = anim_controls::IDX_WALK;
                showcase_render_time = (p % (if walk_dur < 1 { 1 } else { walk_dur })) as f32; // Loop walk if needed
        ...
        if let (Some(model), Some(sheet)) = (&self.held_model, &self.held_sheet) {
            ...
            if self.export_state.is_processing {
                let time_to_use = if self.export_state.showcase_mode {
                    if let Some(anim) = &self.current_anim {
                        if showcase_active_anim_idx == anim_controls::IDX_ATTACK {
                             // Clamp attack to max
                             if showcase_render_time > anim.max_frame as f32 { anim.max_frame as f32 } else { showcase_render_time }
                        } else {
                            // Loops
                ...
                if let Some(anim) = &self.current_anim {
                     anim_exporter::process_frame(ui, rect, &mut self.export_state, model, anim, sheet, self.renderer.clone(), time_to_use);
                }
                ui.ctx().request_repaint();
            }
        ...
        anim_exporter::show_popup(ui, state, model, anim, sheet, show_popup, start_select);
    }
}